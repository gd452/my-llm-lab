...
class Adam:
    ...
    def step(self):
        ...
        for param in self.parameters:
            self.m = self.betas[0] * self.m + (1 - self.betas[0]) * param.grad
            self.v = self.betas[1] * self.v + (1 - self.betas[1]) * param.grad**2
            param.data -= self.lr * self.m / (self.v**0.5 + self.eps)
